  ...
  BEGIN
    IF(clk'EVENT AND clk = '1') THEN
      ...
      CASE state IS
        ...
        WHEN output =>
          --IF(ascii(7) = '0') THEN            --the PS2 code has anASCII output
            ascii_new <= '1';                  --set flag indicating new ASCII output
            ascii_code(6 downto 0) <= ascii(6 DOWNTO 0);   --output the ASCII value
				if break = '0' then -- ADICIONADO
					ascii_code(7) <= '0';
				else  -- ADICIONADO
					ascii_code(7) <= '1'; -- ADICIONADO
				end if; -- ADICIONADO
				break <= '0';
          --END IF;
          state <= ready;                    --return to ready state to await next PS2 code
